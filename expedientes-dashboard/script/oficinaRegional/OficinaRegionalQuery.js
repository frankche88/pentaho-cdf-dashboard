...
OficinaRegionalQuery = {
	//ROUND (ratio_to_report (SUM(CANTIDAD)) OVER (PARTITION BY NOMBRE) * 100, 0) AS PORCENTAJE
    tableExpedientesRegistrados:  function(filtersObject) {
		var filters = this.createFilters(filtersObject, "DOCU");
    ...
    tableExpedientesAtendidos: function(filtersObject) {		
        var filters = this.createFilters(filtersObject, "EXPE");
    ...
    piechartExpedientesAtendidos: function(filtersObject) {
		var filters = this.createFilters(filtersObject, "EXPE");
    ...
    piechartExpedientesRegistrados: function(filtersObject) {
		var filters = this.createFilters(filtersObject, "DOCU");
    ...
    createFilters : function (filtersObject, objFecha) {
        ...
        if(filtersObject){
            ...
            if(filtersObject.startDate != "") {
                consulta += " AND " + objFecha + ".FECHACREACION BETWEEN TO_DATE ('"+filtersObject.startDate+"'||' 00:00', 'yyyy/mm/dd HH24:Mi:ss') ";
            }
