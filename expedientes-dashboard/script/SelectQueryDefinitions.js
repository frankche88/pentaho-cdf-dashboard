	...
	gerenciaQueryDefinition : {
		...
		query : function() {
			var query = " select -1 as idunidad,'TODOS' as nombre from dual"
					...
					+ " select a.idunidad, a.nombre" + " from UNIDAD a"
					+ " where a.dependencia = 231" + " order by 2" + " )";
			console.log(query);
			return query;
		}
	},
	...
	usuarioQueryDefinition : {
		...
		query : function() {
			...
			var query = " select -1 as idusuario,'TODOS' as nombre from dual"
					+ " union all" + " select * from ("
					+ " select a.idusuario, a.nombres" + " from usuario a"
					+ " where a.IDUNIDAD = " + unidad + " order by a.nombres"
					+ " )";
			console.log(query);
			return query;
		}
	...
	flujoQueryDefinition : {
		...
		query : function() {
			var query = " select -1 as idproceso,'TODOS' as nombre from dual"
					+ " union all" + " select * from ("
					+ " select idproceso, nombre" + " from PROCESO"
					+ " order by 2" + " )";
			console.log(query);
			return query;
		}
...
barcharColor = {
	colors : {
		...
		"NO FIRMADO" : "red",
		"FIRMADO" : "green",
		"NO ENUMERADOS Y FIRMADO" : "red",
		"ENUMERADOS Y FIRMADO" : "green",
	},
