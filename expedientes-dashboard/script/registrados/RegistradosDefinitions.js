...
TableDefinitions = {
	...
	registrados : {
		...
		jndi : constants.JNDI,
		query : function() {
			var query = QueryTable.registrados(parameters);
			return query;
		},
		...
		"drawCallback" : function(component) {
			...
			$('#tableRegistradosTable > tbody:first')
					.append(
	...
	enMesaDePartes : {
		colHeaders : [ "Flujo de Documentos", "Mesa de Partes", "Total",
				"Porcentaje" ],
		colTypes : [ 'string', 'numeric', 'numeric', 'numeric' ],
		colFormats : [ null, '%.0f', '%.0f', '%.0f' ],
		colWidths : [ '40%', '20%', '20%', '20%' ],
		...
		jndi : constants.JNDI,
		query : function() {
			var query = QueryTable.enMesaDePartes(parameters);
			return query;
		},
		oLanguage : Tables.oLanguage,
		"drawCallback" : function(component) {
			...
			$('#tableEnMesaDePartesTable > tbody:first')
					.append(
							'<tr><td class="column0 string" style="text-width:bold">Total</td><td class="column1 numeric">'
									+ iTotalNoAtendidos
									+ '</td><td class="column2 numeric">'
	...
	enUsuarioFinal : {
		colHeaders : [ "Flujo de Documentos", "Usuario Final",
				"Total", "Porcentaje" ],
		colTypes : [ 'string', 'numeric', 'numeric', 'numeric' ],
		colFormats : [ null, '%.0f', '%.0f', '%.0f' ],
		colWidths : [ '40%', '20%', '20%', '20%' ],
		...
		jndi : constants.JNDI,
		query : function() {
			var query = QueryTable.enUsuarioFinal(parameters);
			return query;
		},
		oLanguage : Tables.oLanguage,
		"drawCallback" : function(component) {
			...
			$('#tableEnUsuarioFinalTable > tbody:first')
					.append(
							'<tr><td class="column0 string" style="text-width:bold">Total</td><td class="column1 numeric">'
									+ iTotalNoAtendidos
									+ '</td><td class="column2 numeric">'
...
PieChartDefinitions = {
	enMesaDePartes : {
		...
		queryType : 'sql',
		query : function() {
			return QueryPiechart.enMesaDePartes(parameters);
		}
	},
	enUsuarioFinal : {
		width : 440,
		...
		queryType : 'sql',
		query : function() {
			return QueryPiechart.enUsuarioFinal(parameters);
		}
	},
	postFetch : function(values) {
		console.log("postfech:" + values);
